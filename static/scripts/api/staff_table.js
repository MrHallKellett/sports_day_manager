...
export async function fetchStaff(sportsdayId) {
    const res = await fetch(`/sportsdays/${sportsdayId}/staff`);
...
export async function deleteStaff(assignmentId) {
    const res = await fetch(`/staff/assignments/${assignmentId}`, { method: 'DELETE' });
...
export async function uploadStaffCsv(sportsdayId, file) {
    ...
    const res = await fetch(`/sportsdays/${sportsdayId}/staff/upload`, { method: "POST", body: form });
    if (!res.ok) {
        const error = await res.text();
