...
function setupTabs(roles) {
    ...
    tabs.forEach(tab => {
        ...
        const isVisible = (tabName === 'students' && roles.includes('Form Tutor')) ||
                          (tabName === 'events' && roles.includes('Event Steward'));
...
async function loadStudentData(sportsdayId, assignment) { // Pass assignment here
    try {
        ...
        } else if (isEventSteward && assignment.assigned_event_names && assignment.assigned_event_names.length > 0) {
            // Extract unique year groups from assigned events
            const assignedEventYearGroups = new Set();
            assignment.assigned_event_names.forEach(eventName => {
                const match = eventName.match(/Y(\d+)/);
                if (match) {
                    assignedEventYearGroups.add(parseInt(match[1]));
                }
            });
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (assignment.roles.includes('Event Steward') && assignment.assigned_event_names?.length > 0) {
            roleDetails.push(`Event Steward (${assignment.assigned_event_names.join(', ')})`);
        }
        ...
        setupTabs(assignment.roles);

        // An admin might be assigned to a sports day but not have other roles.
        // The student tab should only load if they are a Form Tutor or Event Steward.
        const hasStudentViewRole = assignment.roles.includes('Form Tutor') || assignment.roles.includes('Event Steward');

        if (hasStudentViewRole) {
            await loadStudentData(sports_day.id, assignment); // Pass assignment here
        }
